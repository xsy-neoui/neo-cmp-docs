...
以**下拉列表**选择平台实体对象，在自定义组件中可通过选择的实体对象 key（默认 xObjectApiKey）获取实体业务类型列表、业务对象描述等数据。
...
开发自定义组件时，在 `model.ts` 的 `propsSchema` 中声明后，在渲染或逻辑层从 `props` 读取上述对象并封装实际 HTTP 请求即可。
