...
本文说明自定义组件中如何读取页面/用户/租户信息、组件标识，以及如何通过全局对象 `CTX`（挂载在 `props.env.ctx`）调用系统能力。
...
- 组件标识 **`keyIdentifier`** 在运行/画布侧看 `this.props` 或 `$schema`，属性区看 **`this.props.$com.keyIdentifier`**。  
