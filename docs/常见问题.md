...
## CLI 安装相关 {#cli-install}

### 1. 在 Windows 控制台执行 neo 相关命令出现报错 {#q-windows-script-error}
...
### 2. 在 Windows 控制台执行 neo 相关命令时，提示「无法将 neo 项识别为 cmdlet、函数、脚本文件或可运行程序的名称」 {#q-windows-path-error}
...
### 3. 浏览器没有自动打开？ {#q-browser-not-open}

A：命令行会打印登录授权 URL，将其复制到浏览器中打开即可完成登录和授权。

### 4. Token 刷新失败？ {#q-token-refresh-fail}

A：若 refresh_token 已过期，需重新执行 `neo login`；同时检查网络是否正常。

### 5. 登录后没有正确跳回 redirect_uri？ {#q-redirect-uri-fail}

A：可能被浏览器扩展干扰；已知 **Neo UI Extension** 可能影响授权流程，可暂时关闭后重试。

## 发布相关 {#publish}

### 6. 发布时报错：找不到 `neo-ui-common` 模块 {#q-neo-ui-common-not-found}
...
### 7. 提示「未找到自定义组件模型文件」？ {#q-model-file-not-found}

A：确认存在 `model.ts` / `model.js`，且构建产物中含对应的 `xxCmpModel.js`。

## 设计器与组件渲染相关 {#designer-render}

### 8. 发布后，设计器左侧组件面板看不到自定义组件 {#q-component-not-visible}
...
### 9. 拖拽到画布后未正常渲染（例如只显示空白） {#q-render-blank}
...
### 10. 属性里「是否显示」设为「否」未生效 {#q-visibility-not-work}
...
### 11. 事件动作「组件控制 / 组件可见性」未生效 {#q-event-action-not-work}

**可能原因：** 组件未继承 **BaseCmp**。

「组件控制」「组件可见性」等能力依赖 BaseCmp，请确保自定义组件继承 BaseCmp。


### 12. 已配置的事件动作不执行 {#q-event-action-not-execute}

请依次排查：

1. **是否继承 BaseCmp：** 组件模型中的 `functions`（可被其他组件调用的组件动作）依赖 BaseCmp。
2. **`this` 是否丢失：** 若组件动作内使用 `this`，请在构造函数中进行上下文绑定，例如：`this.refreshData = this.refreshData.bind(this)`，确保执行事件动作时可以正常获取到组件上下文（`this`）。

### 13. 自定义组件根节点样式未生效 {#q-root-style-not-work}

**可能原因：** 根节点没有使用**与组件目录名一致**的类名（className），被样式隔离策略影响。

**说明：** 发布或 `linkDebug` 时，CLI 会为组件样式增加作用域前缀（如 `[data-scope=组件目录名]`）。如果根节点的类名与组件目录名不一致，作用域无法正确匹配，样式就不会生效。

组件根节点请使用**与组件目录名一致**的类名。
