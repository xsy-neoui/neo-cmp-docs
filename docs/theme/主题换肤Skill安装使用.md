...
- 🔍 **硬编码色值替换**：自动识别 18 种品牌相关硬编码色值（主品牌色 9 组 + 次品牌色 9 组，含 6 位 hex、8 位 hex、rgb/rgba 各格式变形），支持完全匹配与仅以 `#0564f5` 为基准的 1-2 色阶近似匹配，替换为 `var(--brand-color)` / `var(--brand-color-hover)` CSS 变量。**执行后自动进行遗漏检测，有遗漏则自动重试直到全部替换完毕**
...
2. 按照匹配规则进行替换：
   - **完全匹配**：色值与对照表完全一致（大小写不敏感）→ 直接替换
   - **近似匹配**：色值仅与主品牌基准色 `#0564f5` 差异在 1-2 色阶内（单通道 ≤ 32 且欧氏距离 ≤ 50）→ 自动推断替换为 `var(--brand-color)`，并在报告中标记 `[近似匹配]` 供人工复核（不与对照表其他色值做近似比较）
...
| 步骤 | 内容 | 涉及数量 |
|:--|:--|:--|
| 硬编码替换 | 主品牌色 9 组 + 次品牌色 9 组 → CSS 变量（含 6 位 hex、8 位 hex、rgb、rgba 格式） | 18 种基准色值 |
...
AI 按固定顺序执行 6 项跳过检查（顺序不可打乱、不可跳步），任一命中即合法跳过：
...
| ③ 已注释行 | 以 `//` 或 `/*` 开头、或被 `/* */` 包裹的行 |
| ④ blue 类选择器（含回溯） | class 名含 `blue` 关键字；所属 CSS 选择器向上回溯（含多行、嵌套、`&` 拼接）后含 `blue` 也跳过 |
...
> **注意**：块内已存在的 `// var(...)` 注释仅跳过其自身，不构成对同块其他活跃行的替换许可——每一行都独立走完上述检查。
