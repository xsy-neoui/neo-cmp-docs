...
2. **执行替换**：按规范逐一替换
   - 硬编码色值 → CSS 变量（自动跳过六类不适用的场景）
   - `rgba()` 色值 → `rgba(var(--xxx-rgb), alpha)` 保留原有透明度
   - 根据属性名（background/border/color/fill）识别元素类型，使用语义化变量
   - 旧 CSS 变量 → 新 CSS 变量
   - 按钮样式 → 按钮色 CSS 变量
...
| 语义化替换 | 根据属性名识别 bg/border/icon/text/button 类型，使用专用变量 | 5 种类型 |
| rgba 替换 | `rgba()` 色值 → `rgba(var(--xxx-rgb), alpha)`，保留原透明度（仅处理 rgba） | — |
...
| class / 变量名含 `blue` | class 名或 SASS/Less 变量名包含 blue 关键字（如 `$blue-base`、`@blue-6`），对应色值跳过 |
| 注释行 | 已注释的代码行 |
| 测试文件 | `*.test.*`、`*.spec.*` 等 |
