...
替换时需分析色值所在 CSS 属性的属性名，推断目标元素类型，选择匹配的语义化 CSS 变量：
...
2. `rgba()` 格式使用对应类型的 **-rgb 伴侣变量**（如 `--color-bg-brand-rgb`）
3. 类型无法明确识别时（如 `box-shadow`），使用主品牌的伴侣变量
4. hover 态使用对应的 `-hover` 后缀变量
...
| `--color-button-filled-primary-secondary-disable` | `168, 215, 255` | ★ 次按钮 — 禁用态背景（比 disable 降一级变浅） |
